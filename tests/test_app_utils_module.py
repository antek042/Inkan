...
def test_search_for_alternatives_returns_app_id_when_found(
    mock_search_for_app, mock_soup_class, mock_get
):
    ...
    fake_headers = [
        MagicMock(attrs={"id": "alt-app-1"}),
        MagicMock(attrs={"id": "alt-app-2"}),
    ]
...
@patch("gazpacho.Soup")
@patch("app_utils.search_for_app")
def test_search_for_alternatives_returns_none_when_not_found(
    mock_search_for_app, mock_soup_class, mock_get
):
